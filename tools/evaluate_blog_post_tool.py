"""Tool for evaluating blog post drafts on accuracy, completeness, readability, and SEO."""
...
class EvaluateBlogPostTool(BaseTool):
    """Evaluates a blog post draft on four criteria using Claude."""
    ...
    input_schema: dict[str, Any] = {
        ...
        'properties': {
            ...
            'content': {
                'type': 'string',
                'description': 'Full HTML blog post body.',
            },
            'excerpt': {
                'type': 'string',
                'description': 'First-pass SEO excerpt.',
            },
            'summaries': {
                'type': 'array',
                'items': {'type': 'object'},
    ...
    def __init__(self) -> None:
        """Initialize with evaluator LLM config."""
        super().__init__(
        ...
        evaluator_config: dict[str, Any] = config['claude_evaluator']
    ...
    def execute(self, input: EvaluateBlogPostInput) -> EvaluateBlogPostOutput:
        """Evaluate a blog post draft and return scores with reasoning.

        Args:
            input: Blog post title, content, excerpt, and ground truth data.

        Returns:
            Evaluation output with scores, reasoning, and suggestions.
        """
    ...
    def _build_prompt(self, input: EvaluateBlogPostInput) -> str:
        """Build the evaluation prompt with blog post and ground truth data.

        Args:
            input: Evaluation input with blog post and source data.
