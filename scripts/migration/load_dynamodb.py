...
def convert_value(value, key: str | None = None):
    ...
    if isinstance(value, dict):
        return {
            k: v
            for k, v in ((k, convert_value(v)) for k, v in value.items())
            if v is not None
        }
...
def convert_item(item: dict) -> dict:
    ...
    return {
        k: v
        for k, v in ((k, convert_value(v, k)) for k, v in item.items())
        if v is not None
    }
...
def load_table(
    ...
    if dry_run:
        ...
        if missing_pk:
            print(
                f"  ERROR: {len(missing_pk)} items missing PK '{pk_field}' at indices: {missing_pk[:5]}"
            )
            return 0
...
def main():
    """Load JSON export files into DynamoDB tables."""
    ...
    parser.add_argument(
        '--dev',
        action='store_true',
        help='Load into dev tables (chicago-sports-recap-dev-*)',
    )
    parser.add_argument(
        '--region', default='us-east-2', help='AWS region (default: us-east-2)'
    )
    parser.add_argument(
        '--tables', nargs='*', help='Only load specific tables (space-separated)'
    )
    parser.add_argument(
    ...
    for table_key in tables_to_load:
        ...
        try:
            written = load_table(
                dynamodb, table_name, items, config['pk'], args.dry_run
            )
