...
class WordPressPublishOutput(BaseModel):
    ...
    categories_resolved: dict[str, int] = Field(
        default_factory=dict, description='Mapping of category name to WordPress ID.'
    )
    tags_resolved: dict[str, int] = Field(
        default_factory=dict, description='Mapping of tag name to WordPress ID.'
    )
    error: Optional[str] = Field(
        default=None, description='Error message if publishing failed.'
    )
