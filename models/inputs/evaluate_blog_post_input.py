...
class EvaluateBlogPostInput(BaseModel):
    ...
    title: str = Field(
        default='', description='Blog post title from create_blog_draft.'
    )
    content: str = Field(
        default='', description='Full HTML blog post body from create_blog_draft.'
    )
    excerpt: str = Field(
        default='', description='First-pass excerpt from create_blog_draft.'
    )
