...
class TestESPNCollector:
    ...
    @patch('builtins.open')
    @patch('utils.article_collectors.api_collectors.espn_collector.rate_limited_request')
    def test_parses_scores_from_espn_response(self, mock_request, mock_open, mock_yaml):
        mock_yaml.return_value = {
            'apis': {'espnapi': {'url': 'http://espn.test', 'chicago_teams': [
        ...
        mock_response.json.return_value = {
            'events': [{
                ...
                'competitions': [{
                    'status': {'type': {'description': 'Final', 'shortDetail': 'Final', 'completed': True}},
                    'competitors': [
                        {'homeAway': 'home', 'team': {'id': '16', 'displayName': 'Chicago Cubs'}, 'score': '5', 'records': [{'type': 'total', 'summary': '8-4'}]},
                        {'homeAway': 'away', 'team': {'id': '8', 'displayName': 'Milwaukee Brewers'}, 'score': '3', 'records': [{'type': 'total', 'summary': '6-6'}]},
                    ],
                    'venue': {'fullName': 'Wrigley Field'},
                    'neutralSite': False,
                    'headlines': [{'description': 'Cubs win', 'shortLinkText': 'Recap'}],
                }],
            }],
        }
        ...
        mock_response.json.return_value = {
            'events': [{
                ...
                'competitions': [{
                    ...
                    'competitors': [
                        {'homeAway': 'home', 'team': {'id': '1', 'displayName': 'Team A'}, 'score': '3'},
                        {'homeAway': 'away', 'team': {'id': '2', 'displayName': 'Team B'}, 'score': '1'},
                    ],
...
class TestNewsAPICollector:
    ...
    @patch('utils.article_collectors.api_collectors.api_collector.yaml.safe_load')
    @patch('builtins.open')
    @patch('utils.article_collectors.api_collectors.newsapi_collector.rate_limited_request')
    def test_collects_and_deduplicates(self, mock_request, mock_open, mock_yaml):
        mock_yaml.return_value = {
            'apis': {'newsapi': {'url': 'http://newsapi.test', 'env_key_name': 'NEWSAPI_KEY', 'language': 'en', 'page_size': 10, 'sort_by': 'publishedAt'}},
        ...
        mock_response.json.return_value = {
            'articles': [
                {'title': 'Cubs win', 'url': 'http://a.com', 'publishedAt': '2026-04-14T00:00:00Z', 'source': {'name': 'ESPN'}, 'content': 'Article text'},
                {'title': 'Cubs win again', 'url': 'http://b.com', 'publishedAt': '2026-04-14T01:00:00Z', 'source': {'name': 'ESPN'}, 'content': 'More text'},
            ],
        }
        mock_response.raise_for_status = MagicMock()
        mock_request.return_value = mock_response
    ...
    def test_skips_removed_content(self, mock_request, mock_open, mock_yaml):
        ...
        mock_response.json.return_value = {
            'articles': [
                {'title': 'Removed article', 'url': 'http://x.com', 'publishedAt': '2026-04-14T00:00:00Z', 'source': {'name': 'X'}, 'content': 'Removed', 'description': ''},
            ],
        }
        mock_response.raise_for_status = MagicMock()
        mock_request.return_value = mock_response
...
class TestSerpApiCollector:
    ...
    def test_collects_news_results(self, mock_client_cls, mock_open):
        api_config = {
            'apis': {'serpapi': {'url': 'http://serp.test', 'env_key_name': 'SERPAPI_KEY'}},
            'teams': [{'name': 'Chicago Cubs'}],
            'collection': {'lookback_hours': 24, 'timeout_seconds': 10},
        }
        ...
        mock_client.search.return_value = {
            'news_results': [
                {'title': 'Cubs trade news', 'link': 'http://a.com', 'iso_date': '2099-04-14T00:00:00Z', 'source': {'name': 'ESPN'}},
            ],
            'sports_results': [],
        }
        mock_client_cls.return_value = mock_client

        with patch('yaml.safe_load', side_effect=[api_config, orch_config]), \
...
class TestSerpApiCollectorRetry:
    ...
    def test_retries_on_429(self, mock_sleep, mock_client_cls, mock_open):
        ...
        mock_client.search.side_effect = [
            ...
            {'news_results': [{'title': 'X', 'link': 'http://x.com', 'iso_date': '2099-01-01T00:00:00Z', 'source': {'name': 'Y'}}], 'sports_results': []},
        ]
        mock_client_cls.return_value = mock_client
    ...
    def test_filters_old_articles_by_cutoff(self, mock_client_cls, mock_open):
        ...
        mock_client.search.return_value = {
            'news_results': [
                {'title': 'Old', 'link': 'http://old.com', 'iso_date': '2020-01-01T00:00:00Z', 'source': {'name': 'X'}},
                {'title': 'New', 'link': 'http://new.com', 'iso_date': '2099-01-01T00:00:00Z', 'source': {'name': 'X'}},
            ],
            'sports_results': [],
        }
        mock_client_cls.return_value = mock_client
...
class TestESPNCollectorEdgeCases:
    ...
    def test_handles_api_error_gracefully(self, mock_request, mock_open, mock_yaml):
        mock_yaml.return_value = {
            'apis': {'espnapi': {'url': 'http://espn.test', 'chicago_teams': [
                {'name': 'Cubs', 'sport': 'baseball', 'league': 'mlb', 'team_id': 16}
            ]}},
            'collection': {'lookback_hours': 24, 'timeout_seconds': 10},
        }
    ...
    def test_returns_none_when_no_home_away(self, mock_request, mock_open, mock_yaml):
        ...
        result = collector._parse_score(
            ...
            competition={'status': {'type': {}}, 'venue': {}, 'neutralSite': False, 'headlines': []},
            competitors=[
                {'homeAway': 'neutral', 'team': {'id': '16', 'displayName': 'Cubs'}, 'score': '0'},
                {'homeAway': 'neutral', 'team': {'id': '8', 'displayName': 'Other'}, 'score': '0'},
            ],
...
class TestNewsAPICollectorEdgeCases:
    ...
    def test_deduplicates_urls_across_teams(self, mock_request, mock_open, mock_yaml):
        ...
        mock_response.json.return_value = {
            'articles': [
                {'title': 'Same article', 'url': 'http://same.com', 'publishedAt': '2026-01-01T00:00:00Z', 'source': {'name': 'X'}, 'content': 'Text'},
            ],
        }
        mock_response.raise_for_status = MagicMock()
        mock_request.return_value = mock_response
...
class TestSerpApiCollectorDedup:
    ...
    def test_deduplicates_urls_within_results(self, mock_client_cls, mock_open):
        ...
        mock_client.search.return_value = {
            'news_results': [
                {'title': 'A', 'link': 'http://same.com', 'iso_date': '2099-01-01T00:00:00Z', 'source': {'name': 'X'}},
                {'title': 'B', 'link': 'http://same.com', 'iso_date': '2099-01-01T00:00:00Z', 'source': {'name': 'Y'}},
            ],
            'sports_results': [],
        }
        mock_client_cls.return_value = mock_client
