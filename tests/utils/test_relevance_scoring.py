...
def _make_tool(scoring_config=None):
    ...
    with patch('tools.fetch_articles_tool.yaml.safe_load', return_value=config), \
         patch('builtins.open'), \
...
class TestRelevanceScoring:
    ...
    def test_credible_source_scores_high(self):
        tool = _make_tool()
        article = {'title': 'Test', 'source': 'ESPN', 'team': 'Cubs', 'publishedAt': None}
        scored = tool._score_article(article)
        assert scored['relevance_score'] > 0

    def test_unknown_source_scores_zero_credibility(self):
        tool = _make_tool()
        article = {'title': 'Test', 'source': 'Random Blog', 'team': 'Cubs', 'publishedAt': None}
        scored = tool._score_article(article)
        credible_article = {'title': 'Test', 'source': 'ESPN', 'team': 'Cubs', 'publishedAt': None}
    ...
    def test_content_signal_keywords_boost_score(self):
        tool = _make_tool()
        with_signal = {'title': 'Cubs trade deadline moves', 'source': 'X', 'team': 'Cubs', 'publishedAt': None}
        without_signal = {'title': 'Cubs game today', 'source': 'X', 'team': 'Cubs', 'publishedAt': None}
        assert tool._score_article(with_signal)['relevance_score'] > tool._score_article(without_signal)['relevance_score']

    def test_team_keyword_in_title_boosts_score(self):
        tool = _make_tool()
        with_team = {'title': 'Cubs win big', 'source': 'X', 'team': 'Cubs', 'publishedAt': None}
        without_team = {'title': 'Big win today', 'source': 'X', 'team': 'Cubs', 'publishedAt': None}
        assert tool._score_article(with_team)['relevance_score'] > tool._score_article(without_team)['relevance_score']
