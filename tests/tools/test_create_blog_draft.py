...
class TestCreateBlogDraftTool:
    ...
    @patch('builtins.open')
    def test_creates_initial_draft(self, mock_open, mock_yaml, mock_claude_cls, mock_scores, mock_summaries, mock_draft):
        mock_yaml.return_value = {'claude_drafter': {'model': 'x', 'temperature': 0.5, 'max_tokens': 4096}}
        ...
        result = tool.execute(CreateBlogDraftInput(summaries=mock_summaries, scores=mock_scores))
    ...
    @patch('tools.create_blog_draft_tool.yaml.safe_load')
    @patch('builtins.open')
    def test_revision_mode_uses_revision_prompt(self, mock_open, mock_yaml, mock_claude_cls, mock_scores, mock_summaries, mock_draft):
        mock_yaml.return_value = {'claude_drafter': {'model': 'x', 'temperature': 0.5, 'max_tokens': 4096}}
        mock_claude = MagicMock()
        mock_claude.send_message.return_value = json.dumps(mock_draft)
        mock_claude_cls.return_value = mock_claude
        ...
        tool = CreateBlogDraftTool()
        tool.claude_client = mock_claude
        tool.execute(CreateBlogDraftInput(
    ...
    @patch('tools.create_blog_draft_tool.yaml.safe_load')
    @patch('builtins.open')
    def test_returns_empty_on_error(self, mock_open, mock_yaml, mock_claude_cls, mock_scores, mock_summaries):
        mock_yaml.return_value = {'claude_drafter': {'model': 'x', 'temperature': 0.5, 'max_tokens': 4096}}
        mock_claude = MagicMock()
...
class TestCreateBlogDraftToolEdgeCases:
    ...
    @patch('tools.create_blog_draft_tool.yaml.safe_load')
    @patch('builtins.open')
    def test_handles_bad_date_in_scores(self, mock_open, mock_yaml, mock_claude_cls, mock_draft):
        mock_yaml.return_value = {'claude_drafter': {'model': 'x', 'temperature': 0.5, 'max_tokens': 4096}}
        mock_claude = MagicMock()
        mock_claude.send_message.return_value = json.dumps(mock_draft)
        mock_claude_cls.return_value = mock_claude
    ...
    @patch('tools.create_blog_draft_tool.yaml.safe_load')
    @patch('builtins.open')
    def test_prompt_with_no_scores_no_summaries(self, mock_open, mock_yaml, mock_claude_cls, mock_draft):
        mock_yaml.return_value = {'claude_drafter': {'model': 'x', 'temperature': 0.5, 'max_tokens': 4096}}
        mock_claude = MagicMock()
        mock_claude.send_message.return_value = json.dumps(mock_draft)
        mock_claude_cls.return_value = mock_claude
    ...
    @patch('tools.create_blog_draft_tool.yaml.safe_load')
    @patch('builtins.open')
    def test_prompt_includes_scheduled_games(self, mock_open, mock_yaml, mock_claude_cls, mock_draft):
        mock_yaml.return_value = {'claude_drafter': {'model': 'x', 'temperature': 0.5, 'max_tokens': 4096}}
        mock_claude = MagicMock()
        mock_claude.send_message.return_value = json.dumps(mock_draft)
        mock_claude_cls.return_value = mock_claude
    ...
    @patch('tools.create_blog_draft_tool.yaml.safe_load')
    @patch('builtins.open')
    def test_rejection_feedback_in_initial_prompt(self, mock_open, mock_yaml, mock_claude_cls, mock_draft):
        mock_yaml.return_value = {'claude_drafter': {'model': 'x', 'temperature': 0.5, 'max_tokens': 4096}}
        mock_claude = MagicMock()
        mock_claude.send_message.return_value = json.dumps(mock_draft)
        mock_claude_cls.return_value = mock_claude
    ...
    @patch('tools.create_blog_draft_tool.yaml.safe_load')
    @patch('builtins.open')
    def test_rejection_feedback_in_revision_prompt(self, mock_open, mock_yaml, mock_claude_cls, mock_draft):
        mock_yaml.return_value = {'claude_drafter': {'model': 'x', 'temperature': 0.5, 'max_tokens': 4096}}
        mock_claude = MagicMock()
        mock_claude.send_message.return_value = json.dumps(mock_draft)
        mock_claude_cls.return_value = mock_claude
