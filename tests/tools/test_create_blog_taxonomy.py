...
class TestCreateBlogTaxonomyTool:
    ...
    @patch('builtins.open')
    def test_assigns_default_category_plus_teams(self, mock_open, mock_yaml, mock_memory_cls):
        mock_yaml.return_value = {'taxonomy': {'default_category': 'Daily Recap', 'max_player_tags': 4}}
        mock_memory = MagicMock()
        mock_memory.get_all_categories.return_value = []
        mock_memory.get_all_tags.return_value = []
        mock_memory.get_or_create_category.side_effect = lambda n: {'id': 1, 'name': n, 'wordpress_id': None}
        mock_memory.get_or_create_tag.side_effect = lambda n: {'id': 1, 'name': n, 'wordpress_id': None}
        mock_memory_cls.return_value = mock_memory

        tool = CreateBlogTaxonomyTool()
        result = tool.execute(CreateBlogTaxonomyInput(
