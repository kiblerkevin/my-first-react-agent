...
class TestSummarizeArticleTool:
    ...
    @patch('builtins.open')
    def test_returns_cached_summary(self, mock_open, mock_yaml, mock_claude_cls, mock_memory_cls):
        mock_yaml.return_value = {'claude_summarizer': {'model': 'x', 'temperature': 0.1, 'max_tokens': 512}}
        cached = {'url': 'http://x.com', 'team': 'Cubs', 'summary': 'Cached.', 'event_type': 'game_recap', 'players_mentioned': [], 'is_relevant': True}
        mock_memory = MagicMock()
        mock_memory.get_article_summary.return_value = cached
        mock_memory_cls.return_value = mock_memory

        tool = SummarizeArticleTool()
        result = tool.execute(SummarizeArticleInput(url='http://x.com', title='Test', team='Cubs'))
    ...
    @patch('tools.summarize_article_tool.yaml.safe_load')
    @patch('builtins.open')
    def test_calls_claude_on_cache_miss(self, mock_open, mock_yaml, mock_claude_cls, mock_memory_cls, mock_request):
        mock_yaml.return_value = {'claude_summarizer': {'model': 'x', 'temperature': 0.1, 'max_tokens': 512}}
        mock_memory = MagicMock()
        mock_memory.get_article_summary.return_value = None
        mock_memory_cls.return_value = mock_memory

        mock_response = MagicMock(status_code=200, text='<html><body><p>Article content here</p></body></html>')
        mock_response.raise_for_status = MagicMock()
        mock_request.return_value = mock_response

        claude_response = json.dumps({'summary': 'New summary.', 'event_type': 'game_recap', 'players_mentioned': ['A'], 'is_relevant': True})
    ...
    @patch('tools.summarize_article_tool.yaml.safe_load')
    @patch('builtins.open')
    def test_marks_irrelevant_when_content_unavailable(self, mock_open, mock_yaml, mock_claude_cls, mock_memory_cls, mock_request):
        mock_yaml.return_value = {'claude_summarizer': {'model': 'x', 'temperature': 0.1, 'max_tokens': 512}}
        mock_memory = MagicMock()
        mock_memory.get_article_summary.return_value = None
        mock_memory_cls.return_value = mock_memory
        ...
        claude_response = json.dumps({'summary': 'Title only.', 'event_type': 'other', 'players_mentioned': [], 'is_relevant': True})
        mock_claude = MagicMock()
        mock_claude.send_message.return_value = claude_response
        mock_claude_cls.return_value = mock_claude
...
class TestSummarizeArticleToolEdgeCases:
    ...
    @patch('tools.summarize_article_tool.yaml.safe_load')
    @patch('builtins.open')
    def test_returns_fallback_on_claude_error(self, mock_open, mock_yaml, mock_claude_cls, mock_memory_cls, mock_request):
        mock_yaml.return_value = {'claude_summarizer': {'model': 'x', 'temperature': 0.1, 'max_tokens': 512}}
        mock_memory = MagicMock()
        mock_memory.get_article_summary.return_value = None
        mock_memory_cls.return_value = mock_memory
        ...
        mock_response = MagicMock(status_code=200, text='<html><body><p>Text</p></body></html>')
        mock_response.raise_for_status = MagicMock()
        mock_request.return_value = mock_response
        ...
        tool = SummarizeArticleTool()
        tool.claude_client = mock_claude
        result = tool.execute(SummarizeArticleInput(url='http://x.com', title='Fallback Title', team='Cubs'))
    ...
    @patch('tools.summarize_article_tool.yaml.safe_load')
    @patch('builtins.open')
    def test_decomposes_html_tags(self, mock_open, mock_yaml, mock_claude_cls, mock_memory_cls, mock_request):
        ...
        claude_response = json.dumps({'summary': 'S', 'event_type': 'other', 'players_mentioned': [], 'is_relevant': True})
        mock_claude = MagicMock()
        mock_claude.send_message.return_value = claude_response
        mock_claude_cls.return_value = mock_claude
