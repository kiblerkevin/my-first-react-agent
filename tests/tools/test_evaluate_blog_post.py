...
class TestEvaluateBlogPostTool:
    ...
    @patch('builtins.open')
    def test_parses_evaluation_response(self, mock_open, mock_yaml, mock_claude_cls, mock_evaluation):
        mock_yaml.return_value = {'claude_evaluator': {'model': 'x', 'temperature': 0.1, 'max_tokens': 2048}}
        ...
        result = tool.execute(EvaluateBlogPostInput(
            title='Test', content='<h1>Test</h1>', excerpt='Test excerpt',
            summaries=[], scores=[],
        ))
    ...
    def test_handles_malformed_response(self, mock_open, mock_yaml, mock_claude_cls):
        ...
        tool.claude_client = mock_claude
        result = tool.execute(EvaluateBlogPostInput(
            title='Test', content='<h1>Test</h1>', excerpt='Test',
            summaries=[], scores=[],
        ))
    ...
    @patch('tools.evaluate_blog_post_tool.yaml.safe_load')
    @patch('builtins.open')
    def test_includes_rejection_feedback_in_prompt(self, mock_open, mock_yaml, mock_claude_cls, mock_evaluation):
        mock_yaml.return_value = {'claude_evaluator': {'model': 'x', 'temperature': 0.1, 'max_tokens': 2048}}
        mock_claude = MagicMock()
        mock_claude.send_message.return_value = json.dumps(mock_evaluation)
        mock_claude_cls.return_value = mock_claude
        ...
        tool = EvaluateBlogPostTool()
        tool.claude_client = mock_claude
        tool.execute(EvaluateBlogPostInput(
            title='Test', content='<h1>Test</h1>', excerpt='Test',
            summaries=[], scores=[], rejection_feedback='Add more detail',
        ))
...
class TestEvaluateBlogPostToolEdgeCases:
    ...
    @patch('tools.evaluate_blog_post_tool.yaml.safe_load')
    @patch('builtins.open')
    def test_handles_string_improvement_suggestions(self, mock_open, mock_yaml, mock_claude_cls):
        mock_yaml.return_value = {'claude_evaluator': {'model': 'x', 'temperature': 0.1, 'max_tokens': 2048}}
        mock_claude = MagicMock()
        # Return suggestions as strings instead of lists
        response = {
            'criteria_scores': {'accuracy': 8, 'completeness': 7, 'readability': 9, 'seo': 6},
            'criteria_reasoning': {'accuracy': 'OK', 'completeness': 'OK', 'readability': 'OK', 'seo': 'OK'},
        ...
        tool.claude_client = mock_claude
        result = tool.execute(EvaluateBlogPostInput(
            title='T', content='C', excerpt='E', summaries=[], scores=[],
        ))
