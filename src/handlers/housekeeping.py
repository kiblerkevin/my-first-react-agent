...
def handler(event, context):
    ...
    memory.update_workflow_run(run_id, {
        "status": "success",
        "steps_completed": ["fetch_scores", "fetch_articles", "deduplicate_articles", "summarize_articles", "draft_and_evaluate", "create_taxonomy", "send_approval_email", "housekeeping"],
...
def _send_drift_alert(alerts: list, email_from: str, email_to: str) -> None:
    ...
    ses.send_email(
        ...
        Message={"Subject": {"Data": f"[Drift Alert] {metric_names}"}, "Body": {"Html": {"Data": html}}},
    )
...
def _send_drift_recovery(recoveries: list, email_from: str, email_to: str) -> None:
    ...
    rows = "".join(f"<tr><td>{r['metric_name']}</td><td>{r.get('value')}</td></tr>" for r in recoveries)
    html = f"""<html><body>
    ...
    ses.send_email(
        Source=email_from,
        Destination={"ToAddresses": [email_to]},
        Message={"Subject": {"Data": f"[Drift Recovered] {metric_names}"}, "Body": {"Html": {"Data": html}}},
    )
