...
def handler(event, context):
    """Fetch, score, and filter articles from all sources."""
    ...
    for source_name, collector in collectors.items():
        try:
            ...
            trimmed = sorted(
                scored, key=lambda a: a.get('relevance_score', 0), reverse=True
            )[:MAX_ARTICLES_PER_SOURCE]
...
def _score_article(article: dict) -> dict:
    ...
    credibility = (
        1.0
        if (article.get('source') or '').lower()
        in [s.lower() for s in CREDIBLE_SOURCES]
        else 0.0
    )
