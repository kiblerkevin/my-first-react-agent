...
from src.shared.config import LLM_SUMMARIZER_MODEL, LLM_SUMMARIZER_TEMPERATURE, LLM_SUMMARIZER_MAX_TOKENS
...
def handler(event, context):
    ...
    for team, articles in by_team.items():
        top = sorted(articles, key=lambda a: a.get("relevance_score", 0), reverse=True)[:max_articles_per_team]
        ...
        for article in top:
            ...
            try:
                ...
                summary_data = {
                    ...
                    "is_relevant": False if content is None else parsed.get("is_relevant", True),
                }
            except Exception:
                summary_data = {"url": url, "team": team, "summary": article.get("title", ""), "event_type": "other", "players_mentioned": [], "is_relevant": False}
...
def _build_prompt(article: dict, team: str, content: str | None) -> str:
    lines = [f"Team: {team}", f"Title: {article.get('title', '')}", f"Published: {article.get('publishedAt', 'unknown')}"]
    if content:
